...
import { DollarSign, MousePointerClick, TrendingUp, Target, Repeat, ShoppingCart } from "lucide-react";
import { AreaChart, Area, Line, LineChart, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from "recharts";
  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <DashboardCard title="Cliques" value={formatNumber(overviewKpis.clicks)} icon={<MousePointerClick />} />
        <DashboardCard title="CPC Médio" value={formatCurrency(overviewKpis.cpc)} icon={<Target />} />
        <DashboardCard title="Conversões" value={formatNumber(overviewKpis.conversions)} icon={<ShoppingCart />} />
        <DashboardCard title="CPA Médio" value={formatCurrency(overviewKpis.cpa)} icon={<Repeat />} />
      ...
      <Card className="bg-card/60 backdrop-blur-sm border-border/30 hover:shadow-neon-blue">
        ...
        <CardContent>
          ...
          <AreaChart data={performanceByDay}>
            ...
            />
            <Area yAxisId="left" type="monotone" dataKey="Custo" stroke="hsl(var(--chart-1))" fill="url(#colorGasto)" strokeWidth={2} />
