...
import { overviewKpis } from "../data";
import { formatNumber } from "@/lib/formatters";
import { cn } from "@/lib/utils";

export const FunnelChart = () => {
    const data = overviewKpis.funnel;
    const maxValue = Math.max(...data.map(d => d.value));
  
    const getPathD = (data: { name: string; value: number }[], width: number, height: number): string => {
    ...
    return (
        <Card className="bg-card/60 backdrop-blur-sm border-border/30 col-span-1 md:col-span-2 lg:col-span-2 flex flex-col justify-between hover:shadow-neon-blue">
            <CardHeader>
            ...
            <ResponsiveContainer width="100%" height="100%">
                ...
                <div className="flex justify-around items-start">
                    ...
                    <div key={index} className="flex-1 text-center">
                        <h3 className="text-sm md:text-base text-blue-200">{item.name}</h3>
                    </div>
                    ))}
                </div>
                ...
                <div className="absolute top-1/2 left-0 w-full h-px" style={{ transform: 'translateY(-50%)' }}>
                    ...
                    {data.slice(0, -1).map((_, index) => (
                        <div key={index} className={cn("w-px bg-blue-300/20 h-full", index === 0 && 'ml-[25%]')}></div>
                    ))}
                    </div>
                </div>
                </div>
            ...
            <CardContent>
                <div className="grid grid-cols-3 gap-2 text-center text-xs">
                    ...
                    <div>
                        ...
                        <p className="font-bold">2,98</p>
                    </div>
                    <div>
                        <p className="text-muted-foreground">CPM</p>
                        <p className="font-bold">R$ 21,81</p>
                    </div>
                </div>
            </CardContent>
        </Card>
